...
const questions = [
    ...
    },
    {
        ...
        answers: ["특별한 준비 없음", "정기 건강 검진만 계획 중", "건강 보험 강화 및 운동 계획 수립", "종합적인 건강관리 계획 및 수립 및 실행 중"],
    },
    {
    ...
    },
    {
        ...
        answers: ["현재 거주 중인 집에서 계속 살 예정", "자녀나 친척과 함께 살 계획", "실버타운이나 은퇴 커뮤니티로 이주 예정", "도심의 편의시설이 갖춰진 곳으로 이사할 계획"],
    },
    {
    ...
    },
    {
        ...
        answers: ["접도 현황 및 진출입로 확보 가능성", "교통 결절점 근접성 및 연계성", "지가 상승 잠재력 및 수익률 전망", "용도지역 현황 및 도시계획 방향성"],
    },
    {
...
노후 준비를 하지 못해서 이런 상황에 처하게 되었지만, 그래도 이곳에서의 추억과 이 시장의 소리가 제게는 큰 위로가 됩니다.`,
    ...
    const CertificationContainer = () => (
        <div>
            <div className="w-full max-w-md mx-auto pb-5 pr-2">
                <img src={`${process.env.PUBLIC_URL}/loginIcon/DAEYEON_Logo_update.png`} alt="Logo" className="ml-2 w-full h-auto" />
            </div>
        </div>
    );
    ...
    return (
        <>
            <Router>
                <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gradient-to-r from-blue-100 to-purple-100" style={{ fontFamily: "'Noto Sans KR', sans-serif" }}>
                    <Routes>
                        <Route path="/login" element={<Login certifiResult={certifiResult} setCertifiResult={setCertifiResult} loginBtn={loginBtn} />}></Route>
                        <Route path="/question" element={<Question {...props} />}></Route>
                        <Route path="/answer" element={<Answer {...props} />}></Route>
        ...
        // <>
        //     <Login certifiResult={certifiResult} setCertifiResult={setCertifiResult} loginBtn={loginBtn}></Login>
        //     {showResult ? (
        //         <Question
